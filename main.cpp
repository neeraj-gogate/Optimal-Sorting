...
int asciiCaseConverter (int ascii)
{
...
int asciiMaker (std::string line, int n)
{ 
  int ascii = (unsigned char)line[n];
  return ascii;
}
  ...
  for (int index=0; index < letterSorter.size(); index++){
    if (letterSorter[index].size() == 1){
      ...
      for (int i=0; i < letterSorter[index].size(); i++){
        ...
        int ascii = asciiMaker(letterSorter[index][i], n);
        ascii = asciiCaseConverter(ascii);
